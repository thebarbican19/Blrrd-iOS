...
-(NSAttributedString *)text:(NSString *)content {
    ...
    if (content != nil) {
        ...
        if (content && !self.noformatting) {
            NSRange range = NSMakeRange(0, content.length);
            [string addAttribute:NSKernAttributeName value:@(1.6) range:range];
            
        }
        else {
            [string addAttribute:NSFontAttributeName value:[UIFont fontWithName:@"Nunito-Bold" size:8.0] range:NSMakeRange(0.0, content.length)];
            [string addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithWhite:0.9 alpha:0.7] range:NSMakeRange(0.0, content.length)];
